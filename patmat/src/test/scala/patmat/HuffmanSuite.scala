...
class HuffmanSuite extends FunSuite {
  ...
  test("test convert()") {
    new TestTrees {
      assert(convert(t1).sortBy(_._1) === List(('a', List(0)),('b', List(1))).sortBy(_._1))
      assert(convert(t2).sortBy(_._1) === List(('a', List(0, 0)),('b', List(0, 1)), ('d', List(1))).sortBy(_._1))
    }
  }

  test("quick encode a very shot text") {
    new TestTrees {
